...
def leaderboard():
    ...
    if all_evaluations_complete:
        ...
        if evaluations:
            total_score = 0  # Store the sum of all evaluator scores
            num_evaluations = len(evaluations)
            
            for evaluation in evaluations:
                # Compute weighted score for each evaluation
                weighted_score = (
                ...
                total_score += weighted_score  # Accumulate scores for all evaluators
            
            # Compute final average score
            participant.score = total_score / num_evaluations
        else:
            participant.score = 0  # Default score if no evaluations exist
